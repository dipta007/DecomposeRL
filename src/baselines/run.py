"""Unified runner for prompted baselines.

Usage:
    PYTHONPATH=. uv run python src/baselines/run.py --method self_ask --dataset pubmedclaim
    PYTHONPATH=. uv run python src/baselines/run.py --method hiss --backend api --provider openai --dataset pubmedclaim
"""
...
def save(
    ...
    """Save per-sample results (JSONL) and aggregated metrics (JSON)."""
...
def main():
    ...
    # Shared
    parser.add_argument(
        "--dataset", "-d", type=str, required=True, help="Dataset name (e.g. pubmedclaim)"
    )
