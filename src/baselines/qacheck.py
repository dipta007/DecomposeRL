...
decomposer/analysis/utils.py pick it up as another "method":
...
Adaptation notes (also in ADAPTATIONS.md):
  - Original QACheck uses a search engine when the model is unsure. We answer
    from the provided evidence only — same closed-evidence regime we use for
    HiSS, FOLK, Chen-2024, and ClaimDecomp.
  - The original demo uses GPT-3.5; we run on the same Qwen / frontier-API
    LLMs as the other prompted baselines for an apples-to-apples comparison.
"""
...
from decomposer.baselines.prompts import extract_verdict_tag
...
# --------------------------------------------------------------------------
# I/O shaped like the other baselines
# --------------------------------------------------------------------------
