...
Pipeline:
  Stage 1 -- Decompose each claim into yes/no sub-questions via the T5
    checkpoint (3B; ~6 GB fp16), taking up to --max_subq questions.
  Stage 2 -- Aggregate sub-question answers with an LLM (vLLM or API) and
    emit a Supported/Refuted verdict.

Adaptations:
  - We replace the original supervised RoBERTa veracity classifier with LLM
    aggregation (same closed-evidence simplification used for Chen-2024).
  - Gold sub-questions are never used; every dataset runs through the T5
    decomposer for consistency.
"""
...
from src.baselines.prompts import build_claimdecomp_aggregator_prompt
...
def process_results(
    ...
    from src.baselines.prompts import aggregate_claimdecomp
