...
Writes  data/combined/step_7_farthest/train.jsonl  (diverse subset)
        data/combined/step_7_farthest/test_*.jsonl  (copied as-is)

Uses farthest-point sampling (MaxMin diversity) for embedding-space
diversity, sqrt-proportional source allocation, and label-balanced
stratification.

Run:  uv run python -m decomposer.data_process.diversify_farthest
"""
...
OUTPUT_DIR = Path("data/combined/step_7_farthest")
...
# ---------------------------------------------------------------------------
# Farthest-point sampling (MaxMin diversity)
# ---------------------------------------------------------------------------
...
def select_diverse(embeddings: np.ndarray, budget: int) -> list[int]:
    ...
    Greedy MaxMin: start from the most central item, then iteratively
    pick the item whose minimum cosine distance to the already-selected
    set is largest.
    """
    ...
    # Seed: item closest to the centroid (most representative start)
    centroid = normed.mean(axis=0)
    centroid_norm = np.linalg.norm(centroid)
    if centroid_norm > 0:
        centroid /= centroid_norm
    first = int(np.argmax(normed @ centroid))

    selected = [first]
    # min_dists[i] = min cosine distance from point i to any selected point
    min_dists = 1.0 - normed @ normed[first]
    min_dists[first] = -1.0  # exclude

    for _ in range(budget - 1):
        next_idx = int(np.argmax(min_dists))
        selected.append(next_idx)
        new_dists = 1.0 - normed @ normed[next_idx]
        min_dists = np.minimum(min_dists, new_dists)
        min_dists[next_idx] = -1.0  # exclude
...
def main():
    ...
    # 5. Run farthest-point selection in parallel across all groups
    ...
    results = list(tqdm(results, total=len(tasks), desc="Farthest-point selection"))
