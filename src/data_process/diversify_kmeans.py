"""Diversity-based train set sampling via KMeans (final curation step).

Reads   data/combined/step_6/*.jsonl
Writes  data/combined/step_7_kmeans/train.jsonl  (diverse subset)
        data/combined/step_7_kmeans/test_*.jsonl  (copied as-is)

Uses KMeans clustering for embedding-space diversity (centroid-nearest
selection), sqrt-proportional source allocation, and label-balanced
stratification.

Run:  uv run python -m decomposer.data_process.diversity_kmeans
"""
...
OUTPUT_DIR = Path("data/combined/step_7_kmeans")
...
def select_diverse(embeddings: np.ndarray, budget: int) -> list[int]:
    """Select diverse subset via KMeans centroid-nearest."""
    ...
    # For each centroid, pick the closest item
    dists = cosine_distances(centroids, embeddings)  # (budget, n)
    selected = set()
    for i in range(budget):
        # Sort by distance, pick the closest that hasn't been selected yet
...
def main():
    ...
    # 5. Run KMeans selection in parallel across all groups
    ...
    results = list(tqdm(results, total=len(tasks), desc="KMeans selection"))
    ...
    print_summary(
        ...
        params={
            ...
            "METHOD": "KMeans centroid-nearest",
        },
    )
